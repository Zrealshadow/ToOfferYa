...
public:
    int GetUglyNumber_Solution(int index) {
        ...
        while(arr.size()<index){
            int new_ele=select_min(arr[pos1]*2,arr[pos2]*3,arr[pos3]*5);
            if(new_ele==arr[pos1]*2)pos1++;
            if(new_ele==arr[pos2]*3)pos2++;
            if(new_ele==arr[pos3]*5)pos3++;
            arr.push_back(new_ele);
        }
        return arr[arr.size()-1];
    }
private:
    int select_min(int x,int y, int z){
...
void test(int k){
    ...
    cout<<m->GetUglyNumber_Solution(k)<<endl;
}
int main() {
    test(1500);
    return 0;
